...
function isMapWithProperty(data: unknown, property: string): boolean {
  ...
  return (
    typeof firstVal === "object" && firstVal !== null && property in firstVal
  );
}
...
export function isDeltasMap(data: unknown): boolean {
  return (
    isMapWithProperty(data, "msgs") && !isMapWithProperty(data, "provider")
  );
}
...
function parseList<T extends string>(
  ...
  for (const item of items) {
    if (!valid.includes(item)) {
      console.error(
        `Error: ${errorPrefix}: ${item}. Valid: ${valid.join(", ")}`,
      );
      process.exit(1);
    }
  }
...
export function parseCollapse(
  value: string | undefined,
): ("sys" | "tool" | "msgs")[] | undefined {
  return parseList(value, ["sys", "tool", "msgs"], "Invalid collapse item");
}

export function parseCollapseBlocks(
  value: string | undefined,
): BlockType[] | undefined {
  return parseList(
    value,
    ["text", "thinking", "td", "tc", "tr", "image", "other"],
    "Invalid block type",
  );
}

export function writeCollapsedExport(
  outputPath: string,
  result: { main: string; blocks: Map<string, string> },
  formatType: string,
): void {
