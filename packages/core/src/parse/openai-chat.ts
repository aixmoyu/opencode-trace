...
function extractMessages(body: Record<string, unknown>): Entry[] {
  ...
  return raw
    .filter(
      (msg: unknown) =>
        !(isRecord(msg) && (msg.role === "system" || msg.role === "developer")),
    )
    .map((msg: unknown): Entry => {
      if (!isRecord(msg))
        return createMsgEntry("user", [createTextBlock(String(msg))]);
...
function extractTools(body: Record<string, unknown>): Entry | undefined {
  ...
  const blocks = raw.map((tool: unknown): Block => {
    if (!isRecord(tool))
      return createToolDefinitionBlock(String(tool), null, null);
