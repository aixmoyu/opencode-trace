  ...
  if (typeof input === "string") {
    ...
    for (const item of input) {
      ...
      if (type === "message" || item.role) {
        const role = String(item.role ?? "user") as
          | "user"
          | "assistant"
          | "tool";
      ...
      for (const tool of body.tools) {
        ...
        if (tool.type === "function" && isRecord(tool.function)) {
          ...
          toolBlocks.push(
            createToolDefinitionBlock(
              "file_search",
              typeof tool.vector_store_ids === "string"
                ? "File search: " + tool.vector_store_ids
                : "File search tool",
              null,
            ),
          );
  ...
  parseResponse(body: unknown): Partial<Conversation> {
    ...
    if (Array.isArray(body.output)) {
      for (const item of body.output) {
        ...
        if (item.type === "message") {
          ...
          msgs.push(
            createMsgEntry("assistant", [
              createOtherBlock({ type: "web_search", status: item.status }),
            ]),
          );
        } else if (item.type === "file_search_call") {
          msgs.push(
            createMsgEntry("assistant", [
              createOtherBlock({ type: "file_search", status: item.status }),
            ]),
          );
        } else if (item.type === "computer_call") {
          msgs.push(
            createMsgEntry("assistant", [
              createOtherBlock({ type: "computer_use" }),
            ]),
          );
        } else {
    ...
    if (isRecord(usage)) {
      ...
      const cachedTokens =
        ...
        typeof (usage.input_tokens_details as Record<string, unknown>)
