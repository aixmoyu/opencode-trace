...
const renderedHtml = computed(() => {
  ...
  if (props.block.type === "td") {
    try {
      ...
      let h = "";
      if (obj.description)
        h += `<div class="tool-def-desc">${esc(obj.description)}</div>`;
      if (obj.parameters)
        h += `<div class="tool-def-params">${formatJsonLines(obj.parameters)}</div>`;
